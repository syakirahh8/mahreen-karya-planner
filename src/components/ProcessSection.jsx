...
export default function ProcessSection() {
  ...
  return (
    <section id="cara-kerja" className="relative py-20 bg-brand-cream border-b-2 border-brand-dark/10">
      {/* Decorative accent */}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-12">
        ...
        <div className="space-y-3 max-w-2xl">
          <div className="inline-flex">
            <span className="px-3.5 py-1 rounded-full bg-white text-brand-dark font-heading font-extrabold text-xs tracking-wider uppercase border-2 border-brand-dark shadow-brutal-sm">
              ALUR BERKARYA
            </span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-heading font-black tracking-tight text-brand-dark">
            Perjalanan Sederhana Menuju Karya Nyata
          </h2>
          <p className="text-sm sm:text-base text-brand-muted font-sans font-medium">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((item) => (
            <div
              key={item.step}
              className={`${item.cardBg} border-2 border-brand-dark rounded-2xl p-6 shadow-brutal flex flex-col justify-between min-h-[250px] hover:-translate-y-1 hover:shadow-brutal-lg transition-all`}
            >
