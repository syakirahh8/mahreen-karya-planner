...
export default function OptionCard({
  ...
  return (
    <motion.button
      ...
      className={`w-full p-4 rounded-2xl border-2 text-left transition-all relative flex items-center justify-between group focus:outline-none focus:ring-2 focus:ring-brand-lavender ${disabled
      ...
      <div className="flex items-center gap-3.5 min-w-0">
        {Icon && (
          <div
            className={`w-10 h-10 rounded-xl border-2 flex items-center justify-center shrink-0 transition-colors ${isSelected
        ...
        <div className="min-w-0">
          <p
            className={`text-sm sm:text-base font-heading font-bold tracking-tight truncate ${isSelected ? 'text-brand-dark font-extrabold' : 'text-brand-dark'
              }`}>
            {label}
          </p>
          {description && (
      ...
      {/* Selected Indicator */}
      <div
        className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center shrink-0 ml-2 transition-all ${isSelected
